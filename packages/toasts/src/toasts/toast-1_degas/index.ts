...
const createAnimatedLayers = (frame: number) => {
  ...
  redLayer.tint(Color.fromHex("#FF0000"));
  redLayer.applyEffect({
    name: "blur",
    options: { radius: getBlurRadius(0.1) },
  });
  redLayer.setOpacity(0.8 * frame);
  ...
  whiteLayer.tint(Color.fromHex("#FFFFFF"));
  ...
  return { purpleLayer, redLayer, whiteLayer };
};

const { purpleLayer, redLayer, whiteLayer } = createAnimatedLayers(1.0);
